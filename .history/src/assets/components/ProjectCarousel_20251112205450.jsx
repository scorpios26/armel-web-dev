  ...
  return (
    <div className="relative w-full max-w-6xl mx-auto">
      ...
      <div className="absolute top-0 left-0 right-0 h-1 bg-white/10 rounded-full overflow-hidden z-20">
        <div
          className="absolute top-0 left-0 h-full bg-gradient-to-r from-violet-500 to-fuchsia-500 transition-all duration-500"
      ...
      <button
        onClick={prevSlide}
        className="nav-button absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center text-white bg-white/10 backdrop-blur-lg hover:bg-white/20 transition-all z-20"
      >
        <svg
      ...
      <button
        onClick={nextSlide}
        className="nav-button absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center text-white bg-white/10 backdrop-blur-lg hover:bg-white/20 transition-all z-20"
      >
        <svg
          className="w-5 h-5 sm:w-6 sm:h-6"
          fill="none"
      ...
      <div className="relative h-[400px] sm:h-[500px] md:h-[600px] overflow-hidden">
        {slides.map((slide, index) => (
          <div
            key={index}
            className={`absolute top-0 left-0 w-full h-full transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] ${
              index === current
